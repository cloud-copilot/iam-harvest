...
import { actionsLocation, conditionKeysLocation, resourceTypesLocation, serviceInfoLocation } from './util/consts.js';

// Get all services
const services = JSON.parse(readFileSync(join(serviceInfoLocation, 'services.json'), 'utf8'));
const serviceNames = JSON.parse(readFileSync(join(serviceInfoLocation, 'serviceNames.json'), 'utf8'));
...
const tests: (() => boolean)[] = [
  ...
  () => {
    ...
    const smoketestServices = ['iam', 'sts', 's3', 'ec2', 'sqs', 'sns', 'lambda', 'dynamodb', 'cloudwatch', 'cloudformation', 'cloudfront', 'cloudtrail', 'logs', 'cognito-identity', 'cognito-sync', 'cognito-idp', 'config', 'datapipeline', 'directconnect', 'dms', 'ds', 'ecr', 'ecs', 'elasticache', 'elasticbeanstalk', 'elasticfilesystem', 'elasticloadbalancing', 'elasticmapreduce', 'elastictranscoder', 'es', 'events', 'firehose', 'gamelift', 'glacier', 'glue', 'greengrass', 'guardduty', 'health', 'iam', 'importexport', 'inspector', 'iot', 'kinesis', 'kinesisanalytics', 'kms', 'lambda', 'lex', 'lightsail', 'machinelearning', 'marketplacecommerceanalytics', 'mediaconvert', 'medialive', 'mediapackage', 'mediastore', 'organizations', 'rds', 'redshift', 'rekognition', 'route53', 's3', 'sdb', 'servicecatalog', 'ses', 'shield', 'sms', 'snowball', 'sns', 'sqs', 'ssm', 'states', 'storagegateway', 'sts', 'support', 'swf', 'waf', 'waf-regional', 'workdocs', 'workspaces', 'xray']
  ...
  () => {
    ...
    for(const resource of expectedResources) {
      ...
      const resources = JSON.parse(readFileSync(join(resourceTypesLocation, `${service}.json`), 'utf8'));
  ...
  () => {
    const expectedConditions = ['s3:ExistingObjectTag/<key>', 'ec2:InstanceType', 'kms:CallerAccount']
    for(const condition of expectedConditions) {
      const [service, conditionKey] = condition.split(':');
      const conditions = JSON.parse(readFileSync(join(conditionKeysLocation, `${service}.json`), 'utf8'));
