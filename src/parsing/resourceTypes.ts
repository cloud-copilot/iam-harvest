...
export function parseResourceTypes(doc: CheerioAPI): ResourceType[]|undefined {
  ...
  // console.log(actionsTable.html());

  return resourceTypes
  // const actions = parseActionsTable(doc, actionsTable)
...
function parseResourceTypeRow(doc: CheerioAPI, row: Cheerio<Element>): ResourceType {
  ...
  if(conditionKeysCell !== '') {
    conditionKeys = doc(cols[2]).find('a').toArray().map((a) => doc(a).text().trim())
  }
...
export function verifyResourceTypesTableAssumptions(doc: CheerioAPI, table: Cheerio<Element>): ProblemRow[] {
  ...
  rows.each((i, el) => {
    ...
    if(row.find('td[rowspan]').length > 0) {
      problemRows.push({problemDescription: 'Rowspan found in resource types table', html: row.html()!})
    }
    if(row.find('td[colspan]').length > 0) {
      problemRows.push({problemDescription: 'Colspan found in resource types table', html: row.html()!})
    }
    if(row.find('td').length !== 3) {
      problemRows.push({problemDescription: 'Row does not have exactly three columns', html: row.html()!})
    }
